...
class Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([conv4, x], dim=1) #######fill in here ####### hint : concatenation (Lecture slides)
        x = self.convUp4(x)
        x = self.upsample(x)
        x = torch.cat([conv3, x], dim=1) #######fill in here ####### hint : concatenation (Lecture slides)
        x = self.convUp3(x)
        x = self.upsample(x)
        x = torch.cat([conv2, x], dim=1) #######fill in here ####### hint : concatenation (Lecture slides)
        x = self.convUp2(x)
        x = self.upsample(x)
        x = torch.cat([conv1, x], dim=1) #######fill in here ####### hint : concatenation (Lecture slides)
        x = self.convUp1(x)
